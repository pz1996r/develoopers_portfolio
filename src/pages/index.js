      ...
      <Layout>
        ...
        <Navigation>
          <List noMobile>
            {NavValues.map((item) => {
              delay += 100;
              return (<Li key={delay} delay={delay} onClick={this.changeLang.bind(null, item.content)}><item.tag active={item.content === this.state.lang}>{item.content + this.state.lang + 'aa'}</item.tag></Li>)
            })}
          </List>
        </Navigation>
        <div id="content">
